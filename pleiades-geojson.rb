...
require 'csv'
require 'json'./pleiades-geojson.rb pleiades-places-latest.csv pleiades-names-latest.csv pleiades-locations-latest.csv
...
places.each_key do |id|
  ...
  unless places[id]["names"].nil?
    places[id]["names"].map{|n| [n["title"],n["nameAttested"],n["nameTransliterated"].split(/,\s*/)]}.flatten.compact.uniq.each do |name|
      names << [name, place_id]
    end
  end
